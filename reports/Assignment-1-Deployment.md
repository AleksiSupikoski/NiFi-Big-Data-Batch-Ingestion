...
Click on “Upload Template” and upload flows from **/code/mysimbdp-dataingest.xml** . Then select the template on top bar and drag & drop it to deploy.
...
<p align="center"><img src="img/controller.png" width="900")<p>
...
> Note the only error that occurs in tihs flow: on the last node. It happens due to the fact that the flow builds a cql insert statement with the header of a csv file. This happens only for the first line though. I have tried to drop the header with a regex, but unsuccesfully, as i am not familiar enough with reqular expression language.
